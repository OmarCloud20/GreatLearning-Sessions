...
The following diagram shows the architecture of the CI/CD pipeline that we will build in this lab. The pipeline will be triggered by pushing a commit to the AWS CodeCommit repository. The pipeline will then build the Docker image and push it to the AWS ECR repository. 


![Architecture Diagram](imgs/lab-1.png)
